...
void TrackGenerator::initializeTracks() {
  ...
  for (int upDown = 0; upDown < 2; upDown++) {
    ...
    while ((upDown == 0 && i >= 0) || (upDown == 1 && i <= _num_azim_2 / 2)) {
      ...
      _num_x[i] *= _geometry->getNumXModules();
      _num_y[i] *= _geometry->getNumYModules();
      
      /*Calculate angle last */
      phi = atan(height * _num_x[i] / (width * _num_y[i]));
...
void TrackGenerator::calculatePenaltyParabola() {
  double tsx,tsy, phi, width, height;
  int middle, nx, ny;
